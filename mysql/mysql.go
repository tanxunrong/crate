...
import (
	...
	"github.com/tanxunrong/crate/mysql/proto"
	"github.com/tanxunrong/crate/sqltypes"
)
...
func (conn *Connection) ExecuteFetch(query string, maxrows int, wantfields bool) (qr *proto.QueryResult, err error) {
	...
	if C.vt_execute(&conn.c, C.CString(query), C.ulong(len(query)), 0) != 0 {
...
func (conn *Connection) ExecuteStreamFetch(query string) (err error) {
	...
	if C.vt_execute(&conn.c, C.CString(query), C.ulong(len(query)), 1) != 0 {
